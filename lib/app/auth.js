module.exports = function(redis) {
    ...
    var cookie = {
        has: function(req) { return util.check.exists(this.get(req)); },
    ...
    var auth = function(req, res, token, succ, fail) {
        tokens.verify(token, function(err, verified) {
            if (util.check.exists(verified)) {
                users.get(token.name, function(err, user) {
                    if (util.check.exists(user)) {
                        tokens.update(token, function(err, updated) {
    ...
    return {
        authenticate: function(req, res, succ, fail) {
            var token;
            if (util.check.exists(req.session.user)) {
                succ();
        ...
        login: function(req, res, succ, fail) {
            ...
            users.authenticate(cred.name, cred.pass, function(err, user) {
                if (util.check.exists(user)) {
                    tokens.create(user.name, function(err, token) {
        ...
        logout: function(req, res, next) {
            req.session.destroy(function() {
                if (cookie.has(req)) {
                    tokens.remove(tokens.parse(cookie.get(req)), function(err) {
                        cookie.del(res);
                        next();
                    });
                } else {
