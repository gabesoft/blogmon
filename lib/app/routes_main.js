module.exports = function(app) {
    ...
    app.get('/posts', authenticate, function(req, res) {
        ...
        //console.log('posts:', start, limit);

        posts.get(userFeeds, start, limit, function(postList) {
            //eyes.inspect(util.pluck(postList, 'date'), start + ' ' + limit);

            settings.populatePostSettings(postList, function(list) {
                // TODO: disable all scripts in posts
                //       see feeds from huffingtonpost.com
                list = list.filter(function(post) {
                    return !util.str.contains(post.description, 'PlayerSeed');
                });
                res.send(list);
            });
        });
