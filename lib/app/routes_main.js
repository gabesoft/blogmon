    ...
    app.get('/posts', authenticate, function(req, res) {
        ...
        users.feeds(user.name, function(uris) {
            feeds.get(uris, function(subscribed) {
                settings.populateFeedSettings(subscribed, function() {
                    var visible = subscribed
                           .filter(function(f) { return f.settings.visible; })
                           .map(function(f) { return f.uri; });
    ...
    app.get('/feeds', authenticate, function(req, res) {
        ...
        users.feeds(user.name, function(uris) {
            feeds.get(uris, function(subscribed) {
                ...
                settings.populateFeedSettings(filtered, function(list) {
                    res.send(list);
                });
            });
        });
