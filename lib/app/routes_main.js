module.exports = function(app, redis, agg) {
    ...
    app.get('/feeds', authenticate, function(req, res) {
        var userFeeds  = req.session.user.feeds,
            subscribes = util.toHash(userFeeds);

        feeds.getAll(function(all) {
    ...
    app.post('/feeds', authenticate, function(req, res) {
        var query = req.body.feed,
            user  = req.session.user;
        finder.find(query, function(feedUrl) {
            console.log('query', query, feedUrl);
            if (util.nil(feedUrl)) {
                res.send({ success: false });
            } else {
                agg.runNow(feedUrl, function(feed) {
                    // TODO: centralize subscribe/unsubscribe
                    feeds.subscribe(feed, function() {
    ...
    app.del('/feeds/:id', authenticate, function(req, res) {
        var uri  = req.params.id,
            user = req.session.user;
        feeds.unsubscribe(uri, function() {
            users.unsubscribe(user.name, uri, function(feeds) {
                user.feeds = feeds;
