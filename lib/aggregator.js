...
var util       = require('./util.js')
  ...
  , buildFeed = function(uri, meta, data) {
        return {
            uri: uri,
            title: meta.title,
            ...
            if (body) {
                parser.parseString(body, function(parseError, meta, rawPosts) {
                    ...
                    var data = {
                      ...
                      , unsorted = rawPosts.map(util.curry(buildPost, feed))
                      , posts    = util
                           .sortBy(unsorted, 'date')
                           .reverse();

                    // TODO: remove line below
                    eyes.inspect(util.pluck2(posts, 'date'));
