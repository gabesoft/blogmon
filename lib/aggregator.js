...
var runFeed = function(agg, feedUri, data, callback) {
    ...
    request(req, function(reqError, res, body) {
        ...
        if (body) {
            ...
            // TODO: fix below (should not get all feeds every time).
            agg.feed.getAll(function(feeds) {
                var selected = feeds.filter(function(f) {
                    return f.uri === req.uri;
                });
                callback(selected[0], 0);
            });
        }
    });
};
...
var runOne = function(agg, feedUri, callback) {
    agg.feed.getData(feedUri, function(data) {
        runFeed(agg, feedUri, data, function(feed, count) {
            if (count === 0) {
                agg.emit('feed-unchanged', feedUri);
            } else {
                agg.emit('feed-updated', feedUri, count);
            }
