var UC_KEY   = 'settings:unread-count'
...
UserSettings.prototype.setUnread = function(feedId, posts, callback) {
    var redis = this.redis;
    this.set({
        type : 'post'
      , name : 'unread'
      ...
      , value: true
        ...
        var count = util.foldl(res, function(acc, x) { return acc + x; }, 0);
        redis.hincrby(UC_KEY, feedId, count, callback);
    });
};

UserSettings.prototype.setRead = function(feedId, posts, callback) {
    var redis = this.redis;
    this.set({
        type : 'post'
      , name : 'unread'
...
UserSettings.prototype.countUnread = function(callback) {
    this.redis.hgetall(UC_KEY, function(err, res) {
        var counts = {};
        Object
           .keys(res)
           .forEach(function(k) {
