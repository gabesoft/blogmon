...
Feed.prototype.subscribe = function(feed, callback) {
  ...
  if (util.check.nil(feed) || util.isEmptyArray(feed)) {
    callback(null, null);
  }
    ...
    redis.sismember(ALL_SET, key, function(err, mem) {
        ...
        multi.incr(subskey(key));
        multi.exec(function(err, res) {
            callback(err, res[res.length - 1]);
        });
    });
  }
};
...
Feed.prototype.update = function(feed, callback) {
  var key = getkey(feed.uri), redis = this.redis;
...
Feed.prototype.unsubscribe = function(uri, callback) {
  var key = getkey(uri), redis = this.redis;
...
Feed.prototype.setData = function(feed, data, callback) {
  var key = getkey(feed.uri), redis = this.redis;
  redis.set(datakey(key), JSON.stringify(data), callback);
};

Feed.prototype.getData = function(feed, callback) {
  var key = getkey(feed.uri), redis = this.redis;
...
Feed.prototype.getall = function(callback) {
  var redis = this.redis, multi = redis.multi();
