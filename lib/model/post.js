var GUID_SET = 'post:guids' 
  ...
  , postKey = function(guid) {
        return util.str.sprintf('posts:%s', guid);
    }
  ...
  , parse = function(str) {
        return JSON.parse(str, function(k, v) {
            return (isDate[k] && util.exists(v)) ? new Date(v) : v;
        });
    }
  ...
  , getRecords = function(redis, posts, callback) {
        var records = []
          , multi   = redis.multi()
          , i       = 0
          , len     = posts.length;
        ...
        posts.forEach(function(post) {
            ...
            multi.exists(postKey(post.guid));
        });

        multi.exec(function(err, res) {
            for (i = 0; i < len; i += 1) {
                records.push({ post: posts[i], exists: !!res[i] });
            }
            callback(records);
        });
  ...
  , addPosts = function(redis, uri, posts, callback) {
        var key   = feedKey(uri)
          , multi = redis.multi();

        getRecords(redis, posts, function(records) {
            var added = [];
            ...
            records.forEach(function(record) {
                ...
                if (!exists) {
                    ...
                    multi.setnx(postKey(post.guid), toStr(post));
...
Post.prototype.add = function(posts, callback) {
    ...
    var redis = this.redis
      ...
      , addfn = function(uri, cont) {
            addPosts(redis, uri, byUri[uri], function(a) {
...
Post.prototype.getPost = function(guid, callback) {
    var redis = this.redis
      , key   = postKey(guid);
    redis.get(key, function(err, res) {
        util.call(callback, parse(res));
    });
};
